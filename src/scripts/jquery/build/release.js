...
var releaseVersion,
	...
	jQueryFilesCDN = [],
	googleFilesCDN = ['jquery.js', 'jquery.min.js', 'jquery.min.map'],
	msFilesCDN = ['jquery-VER.js', 'jquery-VER.min.js', 'jquery-VER.min.map'];
...
function checkGitStatus(next) {
	git(['status'], function(error, stdout, stderr) {
		...
		if (/Changes to be committed/i.test(stdout)) {
			dieIfReal(
				'Please commit changed files before attemping to push a release.'
			);
		}
...
function tagReleaseVersion(next) {
	...
	git(
		['commit', '-a', '-m', 'Tagging the ' + releaseVersion + ' release.'],
		function() {
			...
			if (/\.map$/.test(releaseFile)) {
				...
				text = fs
					...
					.replace(
						/"file":"([^"]+)","sources":\["([^"]+)"\]/,
						'"file":"' +
							unpathedFile.replace(/\.min\.map/, '.min.js') +
							'","sources":["' +
...
function makeArchive(cdn, files, fn) {
	...
	var archive = archiver('zip'),
		...
		output = fs.createWriteStream(
			'dist/' + cdn + '-jquery-' + releaseVersion + '.zip'
		);
...
function exec(cmd, args, fn, skip) {
	if (skip) {
		...
		console.log(cmd + ' ' + args.join(' '));
		child.execFile(cmd, args, { env: process.env }, function(
			err,
			stdout,
			stderr
		) {
