  ...
  _gsScope._gsDefine(
    ...
    function(EventDispatcher, TweenLite, CSSPlugin) {
      var _tempVarsXY = { css: {} }, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
        ...
        _isMultiTouching,
        _isAndroid =
          navigator.userAgent.toLowerCase().indexOf('android') !== -1, //Android handles touch events in an odd way and it's virtually impossible to "feature test" so we resort to UA sniffing
        _lastDragTime = 0,
        ...
        _getDocScrollTop = function() {
          return window.pageYOffset != null
            ...
            : _doc.scrollTop != null
              ? _doc.scrollTop
              : _docElement.scrollTop || _doc.body.scrollTop || 0;
        },
        _getDocScrollLeft = function() {
          return window.pageXOffset != null
            ? window.pageXOffset
            : _doc.scrollLeft != null
              ? _doc.scrollLeft
              : _docElement.scrollLeft || _doc.body.scrollLeft || 0;
        },
        ...
        _getMaxScroll = function(element, axis) {
          ...
          return Math.max(
            0,
            _isRoot(element)
              ? Math.max(_docElement[scroll], body[scroll]) -
                (window['inner' + dim] || _docElement[client] || body[client])
        ...
        _populateIEEvent = function(e, preventDefault) {
          e = e || window.event;
          _tempEvent.pageX =
            e.clientX + _doc.body.scrollLeft + _docElement.scrollLeft;
          _tempEvent.pageY =
            e.clientY + _doc.body.scrollTop + _docElement.scrollTop;
          if (preventDefault) {
            e.returnValue = false;
          }
          return _tempEvent;
        ...
        _unwrapElement = function(value) {
          ...
          return value === window || (value.nodeType && value.style)
            ? value
            : null;
        },
        ...
        _dispatchEvent = function(instance, type, callbackName) {
          ...
          if (typeof callback === 'function') {
            callback.apply(
              vars[callbackName + 'Scope'] || vars.callbackScope || instance,
              vars[callbackName + 'Params'] || [instance.pointerEvent]
            );
          }
          if (listeners) {
        ...
        _transformOriginProp = _checkPrefix(_tempDiv, 'transformOrigin')
          .replace(/^ms/g, 'Ms')
          .replace(/([A-Z])/g, '-$1')
          .toLowerCase(),
        _transformProp = _checkPrefix(_tempDiv, 'transform'),
        _transformPropCSS = _transformProp
          .replace(/^ms/g, 'Ms')
          .replace(/([A-Z])/g, '-$1')
          .toLowerCase(),
        ...
        _isIE10orBelow =
          ...
          parseFloat(RegExp.$1) < 11, //Ideally we'd avoid user agent sniffing, but there doesn't seem to be a way to feature-detect and sense a border-related bug that only affects IE10 and IE9.
        _tempTransforms = [],
        _tempElements = [],
        _getSVGOffsets = function(e) {
          ...
          if (
            Draggable.cacheSVGData !== false &&
            e._gsCache &&
            e._gsCache.lastUpdate === TweenLite.ticker.frame
          ) {
          ...
          if (e.getBBox && !cache.isSVGRoot) {
            ...
            while (
              curElement &&
              (curElement.nodeName + '').toLowerCase() !== 'svg'
            ) {
        ...
        _getOffsetTransformOrigin = function(e, decoratee) {
          ...
          var cs = _getComputedStyle(e),
            v =
              _transformOriginProp && cs
                ? cs.getPropertyValue(_transformOriginProp)
                : '50% 50%',
            a = v.split(' '),
            x =
              v.indexOf('left') !== -1
          ...
          if (e.getBBox && _isSVG(e)) {
            ...
            decoratee.x =
              x.indexOf('%') !== -1
                ? (e.offsetWidth * parseFloat(x)) / 100
                : parseFloat(x);
            decoratee.y =
              y.indexOf('%') !== -1
                ? (e.offsetHeight * parseFloat(y)) / 100
                : parseFloat(y);
          }
          return decoratee;
        },
        _cache = function(e) {
          //computes some important values and stores them in a _gsCache object attached to the element itself so that we can optimize performance
          if (
            Draggable.cacheSVGData !== false &&
            e._gsCache &&
            e._gsCache.lastUpdate === TweenLite.ticker.frame
          ...
          if (isSVGRoot) {
            if (!(cache.offsetParent = e.offsetParent)) {
              //some browsers don't report offsetParent for SVG elements.
        ...
        _getOffset2DMatrix = function(
          ...
          var cache = e._gsCache || _cache(e),
            ...
            parentOffsetParent = cache.isSVG
              ? parentCache.offsetParent
              : parent.offsetParent,
            m,
            isRoot,
            offsets,
          ...
          m =
            ...
            (e.style[_transformProp] + '').indexOf('matrix') !== -1
              ? e.style[_transformProp]
              : cs
                ? cs.getPropertyValue(_transformPropCSS)
                : e.currentStyle
                  ? e.currentStyle[_transformProp]
                  : '1,0,0,1,0,0'; //some browsers (like Chrome 40) don't correctly report transforms that are applied inline on an SVG element (they don't get included in the computed style), so we double-check here and accept matrix values

          if (
            e.getBBox &&
            (e.getAttribute('transform') + '').indexOf('matrix') !== -1
          ) {
          ...
          if (zeroOrigin) {
            ...
            m[4] =
              parseFloat(m[4]) -
              (t.xOrigin - (t.xOrigin * m[0] + t.yOrigin * m[2]));
            m[5] =
              parseFloat(m[5]) -
              (t.yOrigin - (t.xOrigin * m[1] + t.yOrigin * m[3]));
          }
          if (offsetOrigin) {
            if (_svgBorderFactor === undefined) {
              _setEnvironmentVariables();
            ...
            if (cache.isSVG) {
              ...
              borderTranslateX =
                (m[0] - _svgBorderFactor) * borderLeft + m[2] * borderTop;
              borderTranslateY =
                m[1] * borderLeft + (m[3] - _svgBorderFactor) * borderTop;
              ...
              offsetOrigin.x +=
                parseInt(_getStyle(e.offsetParent, 'borderLeftWidth'), 10) || 0;
              offsetOrigin.y +=
                parseInt(_getStyle(e.offsetParent, 'borderTopWidth'), 10) || 0;
            }
            ...
            m[4] =
              ...
              parentOffsetOrigin.x -
              (isRoot ? 0 : parent.scrollLeft || 0);
            m[5] =
              Number(m[5]) +
              offsetOrigin.y +
              (offsets.offsetTop || 0) -
              parentOffsetOrigin.y -
            ...
            if (
              parent &&
              parent !== _docElement &&
              parentOffsetParent === offsets.offsetParent
            ) {
              ...
              if (
                ...
                m[4] -=
                  parseInt(
                    _getStyle(parent.offsetParent, 'borderLeftWidth'),
                    10
                  ) || 0;
                m[5] -=
                  parseInt(
                    _getStyle(parent.offsetParent, 'borderTopWidth'),
                    10
                  ) || 0;
              }
            }
        ...
        _getConcatenatedMatrix = function(e, invert) {
          ...
          var originOffset = _getOffsetTransformOrigin(e, _point1),
            parentOriginOffset = _getOffsetTransformOrigin(
              e.parentNode,
              _point2
            ),
          ...
          while ((e = e.parentNode) && e.parentNode && e !== _docElement) {
            ...
            parentOriginOffset = _getOffsetTransformOrigin(
              e.parentNode,
              originOffset === _point1 ? _point2 : _point1
            );
          ...
          if (e === window) {
            ...
            bottom =
              ...
              ((e.innerHeight || 0) - 20 < _docElement.clientHeight
                ...
                : e.innerHeight || _doc.body.clientHeight || 0); //some browsers (like Firefox) ignore absolutely positioned elements, and collapse the height of the documentElement, so it could be 8px, for example, if you have just an absolutely positioned div. In that case, we use the innerHeight to resolve this.
            ...
            if (e.nodeName.toLowerCase() === 'svg' && !_isOldIE) {
              ...
              if (computedDimensions) {
                ...
                right +=
                  (parseFloat(cs.borderRightWidth) + borderLeft) / cache.scaleX;
                bottom +=
                  (borderTop + parseFloat(cs.borderBottomWidth)) / cache.scaleY;
              }
            }
          }
          if (e === context) {
        ...
        _isTouchDevice =
          'ontouchstart' in _docElement && 'orientation' in window,
        _touchEventLookup = (function(types) {
          //we create an object that makes it easy to translate touch event types into their "pointer" counterparts if we're in a browser that uses those instead. Like IE10 uses "MSPointerDown" instead of "touchstart", for example.
          var standard = types.split(','),
            converted = (_tempDiv.onpointerdown !== undefined
        ...
        _isClickable = function(e) {
          ...
          var data;
          return !e || !e.getAttribute || e.nodeName === 'BODY'
            ? false
            : (data = e.getAttribute('data-clickable')) === 'true' ||
              (data !== 'false' &&
                (e.onclick ||
                  _clickableTagExp.test(e.nodeName + '') ||
                  e.getAttribute('contentEditable') === 'true'))
              ? true
              : _isClickable(e.parentNode);
        },
        ...
        ScrollProxy = function(element, vars) {
          ...
          this.calibrate = function(force) {
            ...
            if (!widthMatches || force) {
              ...
              if (extraPadRight) {
                extraPadRight +=
                  _getStyle(element, 'paddingLeft') +
                  (_addPaddingBR ? _getStyle(element, 'paddingRight') : 0);
              }
            }
        ...
        Draggable = function(target, vars) {
          ...
          var type = (
              vars.type || (_isOldIE ? 'top,left' : 'x,y')
            ).toLowerCase(),
            ...
            render = function(suppressEvents) {
              if (
                self.autoScroll &&
                (checkAutoScrollBounds || (self.isDragging && dirty))
              ) {
                var e = target,
                  ...
                  if (allowY) {
                    ...
                    if (gap < 0) {
                      ...
                      changeY = Math.min(
                        gap,
                        (autoScrollFactor *
                          (1 - Math.max(0, rect.bottom - pointerY) / 40)) |
                          0
                      );
                      ...
                      changeY = -Math.min(
                        parent.scrollTop,
                        (autoScrollFactor *
                          (1 - Math.max(0, pointerY - rect.top) / 40)) |
                          0
                      );
                    }
                    if (changeY) {
                  ...
                  if (allowX) {
                    ...
                    if (gap < 0) {
                      ...
                      changeX = Math.min(
                        gap,
                        (autoScrollFactor *
                          (1 - Math.max(0, rect.right - pointerX) / 40)) |
                          0
                      );
                      ...
                      changeX = -Math.min(
                        parent.scrollLeft,
                        (autoScrollFactor *
                          (1 - Math.max(0, pointerX - rect.left) / 40)) |
                          0
                      );
                    }
                    if (changeX) {
            ...
            calculateBounds = function() {
              ...
              if (scrollProxy) {
                ...
                if (rotationMode) {
                  ...
                  vars.bounds.maxX !== undefined ||
                  vars.bounds.maxY !== undefined
                ) {
                  bounds = vars.bounds;
                  ...
                  self.minX = minX =
                    _getStyle(target, xProp) + bounds.left - targetBounds.left;
                  self.minY = minY =
                    _getStyle(target, yProp) + bounds.top - targetBounds.top;
            ...
            animate = function(throwProps, forceZeroVelocity) {
              ...
              if (throwProps && ThrowPropsPlugin) {
                if (throwProps === true) {
                  snap = vars.snap || {};
                  snapIsRaw =
                    snap instanceof Array || typeof snap === 'function';
                  throwProps = {
                    resistance:
                  ...
                  if (rotationMode) {
                    throwProps.rotation = _parseThrowProps(
                      self,
                      snapIsRaw ? snap : snap.rotation,
                      maxX,
                      minX,
                    ...
                    if (allowX) {
                      throwProps[xProp] = _parseThrowProps(
                        self,
                        snapIsRaw
                          ? snap
                          : snap.x || snap.left || snap.scrollLeft,
                        maxX,
                        minX,
                        scrollProxy ? -1 : 1,
                    ...
                    if (allowY) {
                      throwProps[yProp] = _parseThrowProps(
                        self,
                        snapIsRaw ? snap : snap.y || snap.top || snap.scrollTop,
                        maxY,
                        minY,
                ...
                self.tween = tween = ThrowPropsPlugin.to(
                  scrollProxy || target,
                  {
                  ...
                  isNaN(vars.maxDuration) ? 2 : vars.maxDuration,
                  !isNaN(vars.minDuration)
                    ? vars.minDuration
            ...
            updateMatrix = function() {
              ...
              if (self.isPressed && start.join(',') !== matrix.join(',')) {
                ...
                determinant = a * d - b * c;
                pointerX =
                  startPointerX * (d / determinant) +
                  startPointerY * (-c / determinant) +
                  (c * ty - d * tx) / determinant;
                pointerY =
                  startPointerX * (-b / determinant) +
                  startPointerY * (a / determinant) +
                  -(a * ty - b * tx) / determinant;
                startPointerY =
                  pointerX * matrix[1] + pointerY * matrix[3] + matrix[5];
                startPointerX =
              ...
              if (scrollProxy) {
                ...
                if (rotationMode) {
                  ...
                  startElementY = self.y =
                    Math.atan2(
                      rotationOrigin.y - startPointerY,
                      startPointerX - rotationOrigin.x
                    ) * _RAD2DEG;
                } else {
                  startScrollTop = target.parentNode
                    ? target.parentNode.scrollTop || 0
                    : 0;
                  startScrollLeft = target.parentNode
            ...
            onPress = function(e) {
              ...
              if (_touchEventLookup[e.type]) {
                ...
                touchEventTarget =
                  e.type.indexOf('touch') !== -1
                    ...
                    : _doc; //pointer-based touches (for Microsoft browsers) don't remain locked to the original target like other browsers, so we must use the document instead. The event type would be "MSPointerDown" or "pointerdown".
              ...
              isClicking =
                isClickable.call(self, e.target) && !vars.dragClickables;
              if (isClicking) {
              ...
              if (matrix) {
                temp =
                  startPointerX * matrix[0] +
                  startPointerY * matrix[2] +
                  matrix[4];
                startPointerY =
                  startPointerX * matrix[1] +
                  startPointerY * matrix[3] +
                  matrix[5];
                startPointerX = temp;
              }
              if (self.tween) {
            ...
            onMove = function(e) {
              ...
              if (_isOldIE) {
                ...
                if (
                  ...
                  touchDragAxis =
                    Math.abs(pointerX - startPointerX) >
                      Math.abs(pointerY - startPointerY) && allowX
                      ? 'x'
                      : 'y';
            ...
            onRelease = function(e, force) {
              ...
              if (_placeholderDiv.parentNode) {
                //_placeholderDiv just props open auto-scrolling containers so they don't collapse as the user drags left/up.
              ...
              if (originalEvent && !wasDragging) {
                ...
                if (!_isAndroid || originalEvent.type !== 'touchmove') {
                  ...
                  _dispatchEvent(self, 'click', 'onClick');
                  eventTarget =
                    originalEvent.target || originalEvent.srcElement || target; //old IE uses srcElement
                  clickTime = _getTime();
          ...
          this.getDirection = function(from, diagonalThreshold) {
            ...
            var mode =
                from === 'velocity' && ThrowPropsPlugin
                  ? from
                  : typeof from === 'object' && !rotationMode
                    ? 'element'
                    : 'start',
              xChange,
              yChange,
            ...
            xChange =
              mode === 'start'
                ...
                : mode === 'velocity'
                  ? ThrowPropsPlugin.getVelocity(this.target, xProp)
            ...
            if (rotationMode) {
              ...
              yChange =
                mode === 'start'
                  ...
                  : mode === 'velocity'
                    ? ThrowPropsPlugin.getVelocity(this.target, yProp)
          ...
          this.enable = function(type) {
            ...
            applyObj = {
              ...
              setRatio: _isOldIE
                ...
                : CSSPlugin._internals.setTransformRatio ||
                  CSSPlugin._internals.set3DTransformRatio
            };
            self.update(true);
            return self;
          };
          ...
          if (rotationMode) {
            ...
            tempVars =
              allowX && allowY ? _tempVarsXY : allowX ? _tempVarsX : _tempVarsY;
            cssVars = tempVars.css;
            tempVars.overwrite = false;
          }
      ...
      Draggable.create = function(targets, vars) {
        ...
        var a =
            !targets || targets.length === 0
              ? []
              : _isArrayLike(targets)
                ? _flattenArray(targets)
                : [targets],
          i = a.length;
        while (--i > -1) {
        ...
        _parseRect = function(e, undefined) {
          ...
          if (e === window) {
            ...
            _tempRect.height = _tempRect.bottom =
              (e.innerHeight || 0) - 20 < _docElement.clientHeight
                ? _docElement.clientHeight
                : e.innerHeight || _doc.body.clientHeight || 0;
            return _tempRect;
          }
          var r =
            e.pageX !== undefined
              ...
              : !e.nodeType && e.left !== undefined && e.top !== undefined
                ? e
                : _isOldIE
                  ? _oldIERect(e)
                  : _unwrapElement(e).getBoundingClientRect();
      ...
      Draggable.hitTest = function(obj1, obj2, threshold) {
        ...
        if (isRatio) {
          ...
          return (
            area >= r1.width * r1.height * threshold ||
